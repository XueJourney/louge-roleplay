...
/** 前台站内通知与客服入口展示。调用说明：layout 注入 bootstrap 后自动显示，聊天错误可触发 support 模式。 /
...
| `getPublicCharacterStatsJoinSql` | 内部辅助函数；调用方见所在文件导出或同文件路由/事件处理逻辑。 |
